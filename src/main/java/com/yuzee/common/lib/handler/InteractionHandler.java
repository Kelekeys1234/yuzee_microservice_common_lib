...
public class InteractionHandler {
	...
	private static final String GET_NUMBER_OF_INTERACTION = IConstant.INTERACTION_CONNECTION_URL + "/count/interaction/{entityId}";
	private static final String GET_NUMBER_OF_INTERACTION_FOR_EACH_ENTITY = IConstant.INTERACTION_CONNECTION_URL + "/count/interaction";
	private static final String CHECK_IF_USER_LIKED_ENTITY = IConstant.INTERACTION_CONNECTION_URL + "/user/{userId}/like/entity/{entityId}";
	private static final String CHECK_IF_USER_LIKED_ENTITY_FOR_EACH_ENTITY = IConstant.INTERACTION_CONNECTION_URL + "/user/{userId}/like/entity";
	private static final String GET_NUMBER_OF_VOTES_AND_COMMENT = IConstant.INTERACTION_CONNECTION_URL + "/count/vote/comment/{entityId}";
	...
	public InteractionCountDto getNumberOfInteraction (String entityId) {
		...
		try {
			...
			interactionCountDtoResponse = restTemplate.exchange(path.toString(), HttpMethod.GET, null, new ParameterizedTypeReference<GenericWrapperDto<InteractionCountDto>>() {}, params);
	...
	public List<InteractionCountDto> getNumberOfInteraction (List<String> entityIds){
		...
		try {
			...
			interactionCountDtoResponse = restTemplate.exchange(builder.toUriString(), HttpMethod.GET, null, new ParameterizedTypeReference<GenericWrapperDto<List<InteractionCountDto>>>() {});
			if (interactionCountDtoResponse.getStatusCode().value() != 200) {
				log.error(MSG_ERROR_CODE + interactionCountDtoResponse.getStatusCode().value() );
				throw new InvokeException(MSG_ERROR_CODE + interactionCountDtoResponse.getStatusCode().value() );
			}
	...
	public HasUserLikeDto checkIfUserHasLikedEntity (String userId,String entityId) {
		...
		try {
			...
			hasUserLikeDtoResponse = restTemplate.exchange(path.toString(), HttpMethod.GET, null, new ParameterizedTypeReference<GenericWrapperDto<HasUserLikeDto>>() {}, params);
	...
	public List<HasUserLikeDto> checkIfUserHasLikedEntity (String userId,List<String> entityIds){
		...
		try {
			...
			hasUserLikeDtoResponse = restTemplate.exchange(components.toUriString(), HttpMethod.GET, null, new ParameterizedTypeReference<GenericWrapperDto<List<HasUserLikeDto>>>() {}, params);
			if (hasUserLikeDtoResponse.getStatusCode().value() != 200) {
				log.error(MSG_ERROR_CODE + hasUserLikeDtoResponse.getStatusCode().value() );
				throw new InvokeException(MSG_ERROR_CODE + hasUserLikeDtoResponse.getStatusCode().value() );
			}
	...
	public VoteCommentCountDto getNumberOfVotesAndComment (String entityId) {
		...
		try {
			...
			voteCommentCountResponseDto = restTemplate.exchange(path.toString(), HttpMethod.GET, null, new ParameterizedTypeReference<GenericWrapperDto<VoteCommentCountDto>>() {}, params);
