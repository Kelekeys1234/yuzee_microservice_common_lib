...
public class AuthorizationHandler {
	...
	private static final String AUTHORIZATION_API = IConstant.AUTHORIZATION_BASE_PATH
			+ "/api/v1/provisioning/authorization";
	...
	public void createOrUpdateScopes(GenericRequestWrapper<ScopeDto> scopes) {
		...
		try {
			...
			ResponseEntity<GenericResponse> responseEntity = restTemplate.exchange(SCOPE_API, HttpMethod.PUT, request,
	...
	public void createOrUpdateRoleBasedPolicies(GenericRequestWrapper<RoleBasedPolicyDto> roleBasedPolicies) {
		...
		try {
			...
			ResponseEntity<GenericResponse> responseEntity = restTemplate.exchange(SCOPE_BASED_POLICY_API,
	...
	public void createOrUpdateResources(GenericRequestWrapper<ResourceDto> resources) {
		...
		try {
			...
			ResponseEntity<GenericResponse> responseEntity = restTemplate.exchange(RESOURCE_API, HttpMethod.PUT,
	...
	public void createOrUpdateResourcePermissions(GenericRequestWrapper<ResourcePermissionDto> permissions) {
		...
		try {
			...
			ResponseEntity<GenericResponse> responseEntity = restTemplate.exchange(RESOURCE_PERMISSION_API,
					HttpMethod.PUT, request, GenericResponse.class);
			if (responseEntity.getStatusCode().value() != 200) {
				throw new InvokeException(MSG_ERROR_FROM_AUTH_SERVICE + responseEntity.getStatusCode().value());
			}
